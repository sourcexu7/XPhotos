...
function buildCOSPublicUrl(cfg: Record<string, string>, key: string): string {
  const bucket = cfg['cos_bucket'] || ''
  const endpoint = (cfg['cos_endpoint'] || '').replace(/\/$/, '')
  const useCdn = cfg['cos_cdn'] === 'true'
  const cdnUrl = (cfg['cos_cdn_url'] || '').replace(/\/$/, '')
  const forcePathStyle = cfg['cos_force_path_style'] === 'true'
  ...
  try {
    ...
    switch (storage) {
      ...
      case 'cos': {
        ...
        const requiredKeys = ['cos_secret_id', 'cos_secret_key', 'cos_region', 'cos_endpoint', 'cos_bucket']
        for (const k of requiredKeys) {
          if (!cfg[k]) {
            throw new HTTPException(400, { message: `COS config ${k} is required` })
          }
        }

        const bucket = cfg['cos_bucket']
        const storageFolder = normalizeStorageFolder(cfg['cos_storage_folder'])
        const typeSegment = type && type !== '/' ? String(type).replace(/^\//, '') : ''
        const parts = [storageFolder, typeSegment].filter(Boolean)
        const prefix = parts.length ? parts.join('/') : ''
        const filePath = prefix ? `${prefix}/${filename}` : filename

        const client = getCOSClient(configs)
        const presignedUrl = await generatePresignedUrl(
          client,
          bucket,
          filePath,
...
app.post('/upload', async (c) => {
  ...
  switch (storage.toString()) {
    ...
    case 'cos': {
      try {
        if (!file) {
          throw new HTTPException(400, { message: 'File missing for COS upload' })
        }
        ...
        throw new HTTPException(500, { message: 'COS server upload failed', cause: e })
      }
    }
...
app.post('/getObjectUrl', async (c) => {
  ...
  switch (storage) {
    ...
    case 'cos': {
      ...
      const cfg = toConfigMap(configs)
      const bucket = cfg['cos_bucket'] || ''
      const direct = cfg['cos_direct_download'] === 'true'
      ...
      if (!direct) {
        try {
          ...
          throw new HTTPException(500, { message: 'Failed to sign COS GET URL', cause: e })
        }
      }
...
app.post('/delete', async (c) => {
  try {
    ...
    switch (storage) {
      ...
      case 'cos': {
        const configs = await fetchConfigsByKeys(['cos_secret_id','cos_secret_key','cos_region','cos_endpoint','cos_bucket'])
        const bucket = configs.find((i: Config) => i.config_key === 'cos_bucket')?.config_value || ''
        if (!bucket) throw new HTTPException(400, { message: 'COS bucket 未配置' })
        const client = getCOSClient(configs)
        const { DeleteObjectCommand } = await import('@aws-sdk/client-s3')
        await client.send(new DeleteObjectCommand({ Bucket: bucket, Key: key }))
        return Response.json({ code: 200, message: 'deleted' })
