...
app.get('/validate-cos', async (c) => {
  try {
    ...
    const secretId = getVal('cos_secret_id')
    const secretKey = getVal('cos_secret_key')
    const region = getVal('cos_region')
    const endpoint = getVal('cos_endpoint')
    const bucket = getVal('cos_bucket')
    let storageFolder = getVal('cos_storage_folder')

    if (!secretId || !secretKey || !region || !endpoint || !bucket) {
      throw new HTTPException(400, { message: 'COS 配置不完整，缺少必要字段' })
    }

    // endpoint 需要是 https
    if (!/^https:\/\//i.test(endpoint)) {
      throw new HTTPException(400, { message: 'COS endpoint 必须以 https:// 开头' })
    }
...
app.put('/update-cos-info', async (c) => {
  ...
  const cosEndpoint = query?.find((item: Config) => item.config_key === 'cos_endpoint').config_value
  ...
  const cosForcePathStyle = query?.find((item: Config) => item.config_key === 'cos_force_path_style').config_value
