...
export default function SimpleFileUpload(props: any) {
  ...
  function togglePresetTag(tag: string) {
    ...
    if (imageLabels.includes(tag)) {
      setImageLabels(imageLabels.filter((t: string) => t !== tag))
    } else {
      setImageLabels([...imageLabels, tag])
    }
  }

  // 当选择级联标签时，自动将所选标签加入到标签输入框（不移除已有用户标签）
  useEffect(() => {
    if (!cascaderValue || cascaderValue.length === 0) return
    const [p, ...children] = cascaderValue
    const toAdd: string[] = [p, ...children]
    setPrimarySelect(p)
    setSecondarySelect(children)
    setImageLabels((prev) => {
