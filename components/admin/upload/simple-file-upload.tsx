...
import { getAlistStorages } from '~/lib/api/services'
...
export default function SimpleFileUpload() {
  ...
  const resHandle = useCallback(async (res: UploadResponse, file: File) => {
    try {
      if (album === '/') {
  ...
  const onRequestUpload = useCallback(async (file: File) => {
    // 获取文件名但是去掉扩展名部分
            ...
            >
              <div className="flex flex-col items-center justify-center h-full gap-3 py-8">
                ...
                <div className="text-center space-y-1">
                  <p className="font-semibold text-sm text-gray-900">{t('Upload.dragOrClick')}</p>
                  <p className="text-muted-foreground text-xs">{t('Upload.uploadTipsSingle')}</p>
                </div>
                <div className="flex items-center gap-2 px-4 py-2 bg-blue-50 rounded-lg border border-blue-100">
                  <FileImage className="h-4 w-4 text-blue-600" />
                  <span className="text-xs text-blue-700">支持 JPG、PNG、WebP、HEIC 等格式</span>
                </div>
