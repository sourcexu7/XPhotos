  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
      {data && data.map((album: AlbumType) => (
        <Card key={album.id} className="flex flex-col h-72 show-up-motion items-center gap-0 py-0 bg-white text-gray-900 border-gray-200">
          <div className="flex h-12 justify-start w-full p-2 space-x-2">
            <Badge aria-label={t('Album.albumName')} className="bg-white text-gray-700 border-gray-200 hover:bg-gray-50">{album.name}</Badge>
            <Badge variant="secondary" aria-label={t('Album.router')} className="bg-white text-gray-500 border-gray-200 hover:bg-gray-50">{album.album_value}</Badge>
          </div>
          <div className="flex justify-start w-full p-2 h-48">
            {album.detail || t('Album.noTips')}
          </div>
          <CardFooter className="flex h-12 p-2 mb-1 space-x-1 select-none rounded-md bg-gray-50 border border-gray-100 overflow-hidden py-1 rounded-large w-[calc(100%_-_8px)] shadow-sm z-10">
            <div className="flex flex-1 space-x-1 items-center">
              ...
              <Badge variant="secondary" aria-label={t('Words.sort')} className="bg-white text-gray-500 border-gray-200 hover:bg-gray-50"><ArrowDown10 size={18}/>{album.sort}</Badge>
            </div>
