...
import { addTag, moveTag, checkTagCompleteness } from '~/lib/api/services'
...
export default function TagManager() {
  ...
  const addPrimary = async () => {
    ...
    try {
      const res = await addTag({ name: primaryName.trim() })
      if (res?.code === 200) {
  ...
  const addSecondary = async () => {
    ...
    try {
      const res = await addTag({ name: secondaryName.trim(), parentId: selectedPrimary })
      if (res?.code === 200) {
  ...
  const checkCompleteness = async () => {
    ...
    try {
      const res = await checkTagCompleteness({ fix: true })
