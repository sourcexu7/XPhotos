...
import { getActiveFilterCount, buildSWRKey } from '~/lib/utils/filters'
import { useLoading } from '~/hooks/use-loading'
import { updateImageSort, updateImageAlbum, updateImageFeatured, updateImageShow, getCameraLensList, updateAlbum, getTags } from '~/lib/api/services'
...
export default function ListProps(props : Readonly<ImageServerHandleProps>) {
  ...
  const { data: pageData, error: pageError, isLoading, mutate } = useSWR(
    buildSWRKey(props.args, pageNum, activeFilters),
    async () => {
  ...
  const updateAlbumCover = async (albumValue: string, coverUrl: string) => {
    try {
      ...
      const res = await updateAlbum({ ...targetAlbum, cover: coverUrl })
  ...
  useEffect(() => {
    const loadCameraAndLensList = async () => {
      try {
        const res = await getCameraLensList()
  ...
  async function persistImageSort(nextList: ImageType[]) {
    ...
    try {
      ...
      const res = await updateImageSort({ orders })
      if (res.code !== 200) {
  ...
  async function updateImageAlbum(targetImage: ImageType, albumId: string) {
    ...
    startLoading('updateImageAlbum')
    try {
      const res = await updateImageAlbum({
      ...
      stopLoading('updateImageAlbum')
    }
  }
  
  async function updateImageFeatured(imageId: string, featured: number) {
    startLoading('updateFeatured')
    setUpdateFeaturedId(imageId)
    try {
      const res = await updateImageFeatured({ imageId, featured })
      if (res.code === 200) {
  ...
  async function updateImageShow(imageId: string, show: number) {
    ...
    try {
      const res = await updateImageShow({ imageId, show })
      if (res.code === 200) {
              ...
              localImages.map((img: ImageType, index) => {
                ...
                return (
                  <ImageCard
                    ...
                    updateShowLoading={operationLoading.updateShow}
                    updateShowId={updateShowId}
                    onUpdateFeatured={updateImageFeatured}
                    updateFeaturedLoading={operationLoading.updateFeatured}
                    ...
                    updateImageAlbumLoading={operationLoading.updateImageAlbum}
