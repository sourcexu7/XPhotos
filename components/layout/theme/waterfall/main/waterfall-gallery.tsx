...
import WaterfallImage from '~/components/gallery/waterfall/waterfall-image.tsx'
...
export default function WaterfallGallery(props: Readonly<ImageHandleProps>) {
  ...
  return (
    <div className="w-full min-h-screen bg-white dark:bg-black" ref={containerRef}>
      {/* 瀑布流容器 - 响应式布局，遵循 Ant Design 网格系统 */}
      <div 
        className="
          mx-auto max-w-[1400px]
          px-3 py-4 columns-2 gap-2
          sm:px-4 sm:py-6 sm:columns-2 sm:gap-3
          md:px-6 md:py-8 md:columns-3 md:gap-4
          lg:columns-3 lg:gap-5
        "
      >
        {dataList?.map((item: ImageType) => (
          <WaterfallImage key={item.id} photo={item} />
        ))}
      </div>
