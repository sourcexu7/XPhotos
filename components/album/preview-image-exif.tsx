    ...
    <Dialog>
      ...
      <DialogContent className="sm:max-w-142">
        ...
        <div className="flex flex-col items-start space-x-2">
          {(() => {
            const cam = props.data?.exif?.make ? `${props.data.exif.make} ${props.data.exif.model ?? ''}` : (props.data?.exif?.model ?? '')
            if (!cam) return null
            return (
              <div className="flex items-center space-x-1">
                <CameraIcon className={exifIconClass} size={18} />
                <p className={exifTextClass}>{cam}</p>
              </div>
            )
          })()}
            ...
            <CopyIcon
              ...
              onClick={async () => {
                try {
                  ...
                  // @ts-expect-error - clipboard API typing varies
