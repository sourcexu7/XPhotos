...
export default function GalleryImage({ photo, configData }: { photo: ImageType, configData: any }) {
  ...
  return (
    <div className="w-full" style={{ maxWidth: 1440, margin: '0 auto', padding: '12px 16px' }}>
      ...
      <div>
        {photo.title && (
          <Title level={5} style={{ margin: '0 0 8px 0', fontWeight: 500 }}>
            {photo.title}
          </Title>
        )}
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px 12px', fontSize: '12px', color: '#666', marginBottom: '8px' }}>
          {(() => {
      ...
      <Row gutter={[24, 0]} align="top">
        <Col xs={24} sm={24} md={24} lg={5} xl={4}>
          ...
          <div style={{ marginTop: '16px', paddingLeft: '32px' }}>
            <Text type="secondary" style={{ fontSize: 13, fontWeight: 500 }}>拍摄参数</Text>
            <div style={{ width: '88px', height: '1px', backgroundColor: '#d9d9d9', marginTop: '6px' }} />
          </div>
          <div style={{ marginTop: '12px' }}>
              ...
              return showCam ? (
                <div style={{ marginBottom: '16px', display: 'flex', alignItems: 'flex-start', gap: '10px', marginLeft: '32px' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '3px' }}>
                    <Text type="secondary" style={{ fontSize: 12 }}>相机型号</Text>
                    <Text style={{ fontSize: 13 }}>{showCam}</Text>
                  </div>
                </div>
              ) : null
            })()}
            {photo?.exif?.data_time && (
              <div style={{ marginBottom: '16px', display: 'flex', alignItems: 'flex-start', gap: '10px', marginLeft: '32px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '3px' }}>
                  <Text type="secondary" style={{ fontSize: 12 }}>拍摄日期</Text>
            ...
            {photo?.exif?.f_number && (
              <div style={{ marginBottom: '16px', display: 'flex', alignItems: 'flex-start', gap: '10px', marginLeft: '32px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '3px' }}>
                  <Text type="secondary" style={{ fontSize: 12 }}>光圈</Text>
                  <Text style={{ fontSize: 13 }}>{photo?.exif?.f_number}</Text>
                </div>
              </div>
            )}
            {photo?.exif?.exposure_time && (
              <div style={{ marginBottom: '16px', display: 'flex', alignItems: 'flex-start', gap: '10px', marginLeft: '32px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '3px' }}>
                  <Text type="secondary" style={{ fontSize: 12 }}>快门速度</Text>
                  <Text style={{ fontSize: 13 }}>{photo?.exif?.exposure_time}</Text>
                </div>
              </div>
            )}
            {photo?.exif?.focal_length && (
              <div style={{ marginBottom: '16px', display: 'flex', alignItems: 'flex-start', gap: '10px', marginLeft: '32px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '3px' }}>
                  <Text type="secondary" style={{ fontSize: 12 }}>焦距</Text>
                  <Text style={{ fontSize: 13 }}>{photo.exif.focal_length}</Text>
                </div>
              </div>
            )}
            {photo?.exif?.iso_speed_rating && (
              <div style={{ marginBottom: '16px', display: 'flex', alignItems: 'flex-start', gap: '10px', marginLeft: '32px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '3px' }}>
                  <Text type="secondary" style={{ fontSize: 12 }}>ISO</Text>
                  <Text style={{ fontSize: 13 }}>{photo.exif.iso_speed_rating}</Text>
                </div>
              </div>
            )}
          </div>
          ...
          {photo?.labels && photo.labels.length > 0 && (
            <div style={{ marginTop: '12px', paddingLeft: '32px', display: 'flex', flexWrap: 'wrap', gap: '0px' }}>
              {photo.labels.map((tag: string) => (
                <Tag
                  key={tag}
                  className="cursor-pointer select-none"
